...
内置 Dashboard、最佳实践、设置、用户、关于和登录页面，支持按用户账号 —— 无需外部前端框架。

## 快速开始

1. 启动服务器：`docker-compose up -d`
2. 使用默认账号（`admin` / `admin`）登录，并立即修改密码
3. 安装客户端：`pip install jinjinxia-client`
4. 为您的 AI 平台安装钩子：`jinjinxia-client install <代理> --url <URL> --key <KEY>`
5. 在 `http://<服务器>:8080/admin` 配置设置
6. 照常使用 AI 技能 —— 改进自动发生

## 隐私

- **所有数据留在您的基础设施内** — 自托管服务器、本地数据库、无云依赖
- **内置账号，无第三方 SaaS** — 自托管账号系统保护 Web 界面与消息 API；无需在外部服务注册、无需订阅
